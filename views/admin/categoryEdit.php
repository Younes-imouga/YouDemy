  ...
  <main class="flex-grow container mx-auto mt-8">
    ...
    <form action="/admin/edit-category" method="POST" class="bg-white shadow rounded-lg p-8 w-1/2 mx-auto">
      ...
      <div class="mb-6">
        ...
        <input type="text" id="category-name" name="name" class="w-full mt-2 p-3 border border-gray-300 rounded" value="<?php echo htmlspecialchars($data['category']['name']); ?>" required>
      </div>
      <div class="mb-6">
        <label for="category-description" class="block text-sm font-medium text-gray-700">Description</label>
        <textarea id="category-description" name="description" class="w-full mt-2 p-3 border border-gray-300 rounded" rows="4"><?php echo htmlspecialchars($data['category']['description']); ?></textarea>
      </div>
